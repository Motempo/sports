      ...
      <div className="space-y-1">
        {group.rows.map((row) => (
          ...
          >
            ...
            <TeamEmblem team={row.team} size={20} />
            <span className="min-w-0 flex-1 truncate text-[13px] font-semibold">{row.team.name}</span>
            <span className="shrink-0 text-[11px] tabular-nums text-muted sm:text-[12px]">
              {row.played}P {row.won}-{row.drawn}-{row.lost}
            </span>
...
export function GroupStandingsGrid({ standings }: GroupStandingsGridProps) {
  ...
  return (
    <div>
      <div className="scrollbar-hide mb-4 flex gap-1 overflow-x-auto border-b border-border pb-1 lg:hidden">
        {standings.map((g) => (
          <button
            key={g.groupId}
            type="button"
