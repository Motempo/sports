...
- Next-event card: Wikipedia / Wikimedia Commons aerial or satellite photography (45° oblique / overhead); curated seed in `data/venue-aerial-images.json`; local generated fallback in `public/venues/aerial/` when no free photo exists
- F1 uses the circuit aerial; football uses the match stadium or the home club’s ground
