...
const nameAliases: Record<string, string> = {
  ...
  tottenham: "TOT",
  spurs: "TOT",
  // Recent relegations — keep aliases so older feeds still resolve.
  "burnley fc": "BUR",
  burnley: "BUR",
...
export function buildClubTeamInfo(
  code: string,
  name?: string,
  crest?: string,
  shortName?: string
...
export function resolveClubCode(teamName: string, tla?: string | null): string {
  if (tla?.trim()) {
...
export function computeLeagueStandings(
  matches: MatchInfo[],
  seasonLabel: string,
  seedTeams: TeamInfo[] = clubs.map((c) => buildClubTeamInfo(c.code, c.name, c.crest, c.shortName))
): LeagueStandings {
...
export function computeTitleRace(
  standings: LeagueStandings,
  leagueName = "Premier League"
): PremierLeagueRaceInsight | null {
...
export function computeRelegationRace(
  standings: LeagueStandings
): PremierLeagueRaceInsight | null {
