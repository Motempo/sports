...
export function showStandingsPrimary(phase: PremierLeaguePhase): boolean {
  return phase === "EARLY" || phase === "MID" || phase === "RUN_IN" || phase === "COMPLETE";
}

export function getPremierLeagueWhatsNext(phase: PremierLeaguePhase, matchday: number): string {
  switch (phase) {
    case "PRE":
      return "Kickoff is around the corner — the full 38-matchday calendar is locked in.";
    case "EARLY":
      return `Matchday ${Math.min(TOTAL_MATCHDAYS, matchday + 1)} is next — every early result reshapes Europe and relegation maths.`;
    case "MID":
    ...
    case "COMPLETE":
      return "Season complete — champions crowned, Europe places filled, three clubs relegated.";
  }
}

export function buildPremierLeagueRailSteps(): PremierLeagueRailStep[] {
  return [
...
export function getActivePremierLeagueRailStep(phase: PremierLeaguePhase): string {
  return phase;
}
